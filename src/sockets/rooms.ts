...
import { TimerEvents, ChatEvents } from './events';
...
export async function respondChatForUserDevice(
  server: Server,
  userId: string,
  deviceId: string,
  message: string,
  ...
  sendToUserDevice(server, userId, deviceId, ChatEvents.typing.toString(), {
    status: "typing",
  });
  apiService.getBotResponse(message).then((botResponse) => {
    console.log(botResponse)
    sendToUserDevice(server, userId, deviceId, ChatEvents.received.toString(), {
      message: botResponse,
    });
  }, (err)=>{
